...
if (isset($_POST['simpan'])) {
    ...
    if ($nama == '' or $isi == '') {
        $error = "Silahkan masukan semua data isi yakni adalah data isi dan nama";
    }
    //print_r($_FILES);
    if($_FILES['foto']['name']){
    ...
    if (empty($error)) {
        if($foto_name){
            ...
            $foto_name = "partnersi_".time()."_".$foto_name;
        ...
        if($id != ""){
            $sql1  = "update partnersi set nama ='$nama',foto='$foto_name',isi='$isi',tgl_isi=now() where id ='$id'";
        }else{
            $sql1 = "insert into partnersi(nama,foto,isi) values ('$nama','$foto_name','$isi')";
        }
