...
if (isset($_POST['simpan'])) {
    ...
    if ($judul == '' or $isi == '') {
        $error = "Silahkan masukan semua data isi yakni adalah data isi dan judul";
    }

    if (empty($error)) {
        if($id != ""){
            $sql1  = "update halamani set judul ='$judul',kutipan= '$kutipan',isi='$isi',tgl_isi=now() where id ='$id'";
        }else{
            $sql1 = "insert into halamani(judul,kutipan,isi) values ('$judul','$kutipan','$isi')";
        }
